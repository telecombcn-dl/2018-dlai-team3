...
parser.add_argument('--dataset', required=False, default='horse2zebra',
                    help='')
parser.add_argument('--train_subfolder', required=False, default='train',
                    help='')
parser.add_argument('--test_subfolder', required=False, default='test',
                    help='')
parser.add_argument('--input_ngc', type=int, default=3,
                    help='input channel for generator')
parser.add_argument('--output_ngc', type=int, default=3,
                    help='output channel for generator')
parser.add_argument('--input_ndc', type=int, default=3,
                    help='input channel for discriminator')
parser.add_argument('--output_ndc', type=int, default=1,
...
parser.add_argument('--resize_scale', type=int, default=286,
                    help='resize scale (0 is false)')
parser.add_argument('--crop', type=bool, default=True,
                    help='random crop True or False')
...
parser.add_argument('--decay_epoch', type=int, default=100,
                    help='learning rate decay start epoch num')
parser.add_argument('--lrD', type=float, default=0.0002,
                    help='learning rate, default=0.0002')
parser.add_argument('--lrG', type=float, default=0.0002,
                    help='learning rate, default=0.0002')
parser.add_argument('--lambdaA', type=float, default=10,
                    help='lambdaA for cycle loss')
parser.add_argument('--lambdaB', type=float, default=10,
                    help='lambdaB for cycle loss')
parser.add_argument('--beta1', type=float, default=0.5,
                    help='beta1 for Adam optimizer')
parser.add_argument('--beta2', type=float, default=0.999,
                    help='beta2 for Adam optimizer')
parser.add_argument('--save_root', required=False, default='results',
...
train_loader_A = util.data_load('data/' + opt.dataset,
                                opt.train_subfolder + 'A', transform,
                                opt.batch_size, shuffle=True)
train_loader_B = util.data_load('data/' + opt.dataset,
                                opt.train_subfolder + 'B', transform,
                                opt.batch_size, shuffle=True)
test_loader_A = util.data_load('data/' + opt.dataset, opt.test_subfolder + 'A',
                               transform, opt.batch_size, shuffle=False)
test_loader_B = util.data_load('data/' + opt.dataset, opt.test_subfolder + 'B',
                               transform, opt.batch_size, shuffle=False)

# network
G_A = generator(opt.input_ngc, opt.output_ngc, opt.ngf)
G_B = generator(opt.input_ngc, opt.output_ngc, opt.ngf)
D_A = discriminator(opt.input_ndc, opt.output_ndc, opt.ndf)
D_B = discriminator(opt.input_ndc, opt.output_ndc, opt.ndf)
G_A.cuda()
G_B.cuda()
D_A.cuda()
D_B.cuda()
...
G_optimizer = optim.Adam(itertools.chain(G_A.parameters(), G_B.parameters()),
                         lr=opt.lrG, betas=(opt.beta1, opt.beta2))
D_A_optimizer = optim.Adam(D_A.parameters(), lr=opt.lrD,
                           betas=(opt.beta1, opt.beta2))
D_B_optimizer = optim.Adam(D_B.parameters(), lr=opt.lrD,
                           betas=(opt.beta1, opt.beta2))
...
for epoch in range(opt.train_epoch):
    ...
    if (epoch + 1) > opt.decay_epoch:
        D_A_optimizer.param_groups[0]['lr'] -= opt.lrD / (
                opt.train_epoch - opt.decay_epoch)
        D_B_optimizer.param_groups[0]['lr'] -= opt.lrD / (
                opt.train_epoch - opt.decay_epoch)
        G_optimizer.param_groups[0]['lr'] -= opt.lrG / (
                opt.train_epoch - opt.decay_epoch)
    ...
    for (realA, _), (realB, _) in zip(train_loader_A, train_loader_B):
        ...
        train_hist['G_A_losses'].append(G_A_loss)
        train_hist['G_B_losses'].append(G_B_loss)
        train_hist['A_cycle_losses'].append(A_cycle_loss)
        train_hist['B_cycle_losses'].append(B_cycle_loss)
        G_A_losses.append(G_A_loss)
        G_B_losses.append(G_B_loss)
        A_cycle_losses.append(A_cycle_loss)
        B_cycle_losses.append(B_cycle_loss)

        # train discriminator D_A
        D_A_optimizer.zero_grad()

        D_A_real = D_A(realB)
        D_A_real_loss = MSE_loss(D_A_real,
                                 Variable(torch.ones(D_A_real.size()).cuda()))

        # fakeB = fakeB_store.query(fakeB.data)
        fakeB = fakeB_store.query(fakeB)
        D_A_fake = D_A(fakeB)
        D_A_fake_loss = MSE_loss(D_A_fake,
                                 Variable(torch.zeros(D_A_fake.size()).cuda()))

        D_A_loss = (D_A_real_loss + D_A_fake_loss) * 0.5
        D_A_loss.backward()
        D_A_optimizer.step()

        train_hist['D_A_losses'].append(D_A_loss)
        ...
        D_B_real = D_B(realA)
        D_B_real_loss = MSE_loss(D_B_real,
                                 Variable(torch.ones(D_B_real.size()).cuda()))

        # fakeA = fakeA_store.query(fakeA.data)
        fakeA = fakeA_store.query(fakeA)
        D_B_fake = D_B(fakeA)
        D_B_fake_loss = MSE_loss(D_B_fake,
                                 Variable(torch.zeros(D_B_fake.size()).cuda()))

        D_B_loss = (D_B_real_loss + D_B_fake_loss) * 0.5
        D_B_loss.backward()
        D_B_optimizer.step()

        train_hist['D_B_losses'].append(D_B_loss)
    ...
    if (epoch + 1) % 10 == 0:
        ...
        for realA, _ in test_loader_A:
            n += 1
            path = opt.dataset + '_results/test_results/AtoB/' + str(
                epoch) + '_' + str(
                n) + '_input.png'
            ...
            path = opt.dataset + '_results/test_results/AtoB/' + str(
                ...
                n) + '_output.png'
            plt.imsave(path,
                       (genB[0].cpu().data.numpy().transpose(1, 2, 0) + 1) / 2)
            recA = G_B(genB)
            path = opt.dataset + '_results/test_results/AtoB/' + str(
                epoch) + '_' + str(
                n) + '_recon.png'
            plt.imsave(path,
                       (recA[0].cpu().data.numpy().transpose(1, 2, 0) + 1) / 2)

        # test B to A
        n = 0
        for realB, _ in test_loader_B:
            n += 1
            path = opt.dataset + '_results/test_results/BtoA/' + str(
                epoch) + '_' + str(
                n) + '_input.png'
            ...
            path = opt.dataset + '_results/test_results/BtoA/' + str(
                ...
                n) + '_output.png'
            plt.imsave(path,
                       (genA[0].cpu().data.numpy().transpose(1, 2, 0) + 1) / 2)
            recB = G_A(genA)
            path = opt.dataset + '_results/test_results/BtoA/' + str(
                epoch) + '_' + str(
                n) + '_recon.png'
            plt.imsave(path,
                       (recB[0].cpu().data.numpy().transpose(1, 2, 0) + 1) / 2)
    else:
        n = 0
        for realA, _ in train_loader_A:
            n += 1
            path = opt.dataset + '_results/train_results/AtoB/' + str(
                epoch) + '_' + str(
                n) + '_input.png'
            plt.imsave(path, (realA[0].numpy().transpose(1, 2, 0) + 1) / 2)
            realA = Variable(realA.cuda(), volatile=True)
        ...
        for realB, _ in train_loader_B:
            n += 1
            path = opt.dataset + '_results/train_results/BtoA/' + str(
                epoch) + '_' + str(
                n) + '_input.png'
            plt.imsave(path, (realB[0].numpy().transpose(1, 2, 0) + 1) / 2)
            realB = Variable(realB.cuda(), volatile=True)
