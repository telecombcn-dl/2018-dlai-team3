...
class CYCLEGAN(object):
    def __init__(self, args):
        ...
        # load dataset
        self.data_loaderA = dataloader(self.datasetA, self.input_size,
                                       self.batch_size)
        dataA = self.data_loaderA.__iter__().__next__()[0]
        self.data_loaderB = dataloader(self.datasetB, self.input_size,
                                       self.batch_size)
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for (realA, _), (realB, _) in itertools.izip(self.data_loaderA,
                                                         self.data_loaderB):
                ...
                D_A_real_loss = self.BCE_loss(D_A_real, Variable(
                    torch.ones(D_A_real.size()).cuda()))

                # fakeB = fakeB_store.query(fakeB.data)
                fakeB = fakeB_store.query(fakeB)
                D_A_fake = self.D_A(fakeB)
                D_A_fake_loss = self.BCE_loss(D_A_fake, Variable(
                    torch.zeros(D_A_fake.size()).cuda()))
                ...
                D_B_real_loss = self.BCE_loss(D_B_real, Variable(
                    torch.ones(D_B_real.size()).cuda()))

                # fakeA = fakeA_store.query(fakeA.data)
                fakeA = fakeA_store.query(fakeA)
                D_B_fake = self.D_B(fakeA)
                D_B_fake_loss = self.BCE_loss(D_B_fake, Variable(
                    torch.zeros(D_B_fake.size()).cuda()))
                ...
                if (epoch + 1) % 10 == 0:
                    ...
                    for realA, _ in test_loader_A:
                        ...
                        path = opt.dataset + '_results/test_results/AtoB/' + str(
                            n) + '_input.png'
                        plt.imsave(path, (
                            realA[0].numpy().transpose(1, 2, 0) + 1) / 2)
                        realA = Variable(realA.cuda(), volatile=True)
                        genB = self.G_A2B(realA)
                        path = opt.dataset + '_results/test_results/AtoB/' + str(
                            n) + '_output.png'
                        plt.imsave(path, (
                            genB[0].cpu().data.numpy().transpose(1, 2,
                                                                 0) + 1) / 2)
                        recA = self.G_B2A(genB)
                        path = opt.dataset + '_results/test_results/AtoB/' + str(
                            n) + '_recon.png'
                        plt.imsave(path, (
                            recA[0].cpu().data.numpy().transpose(1, 2,
                                                                 0) + 1) / 2)
                    ...
                    for realB, _ in test_loader_B:
                        ...
                        path = opt.dataset + '_results/test_results/BtoA/' + str(
                            n) + '_input.png'
                        plt.imsave(path, (
                            realB[0].numpy().transpose(1, 2, 0) + 1) / 2)
                        realB = Variable(realB.cuda(), volatile=True)
                        genA = self.G_B2A(realB)
                        path = opt.dataset + '_results/test_results/BtoA/' + str(
                            n) + '_output.png'
                        plt.imsave(path, (
                            genA[0].cpu().data.numpy().transpose(1, 2,
                                                                 0) + 1) / 2)
                        recB = self.G_A2B(genA)
                        path = opt.dataset + '_results/test_results/BtoA/' + str(
                            n) + '_recon.png'
                        plt.imsave(path, (
                            recB[0].cpu().data.numpy().transpose(1, 2,
                                                                 0) + 1) / 2)
                else:
                    n = 0
            ...
            torch.save(self.G_A2B.state_dict(), root + model + 'generatorA_param.pkl')
            torch.save(self.G_B2A.state_dict(), root + model + 'generatorB_param.pkl')
            torch.save(D_A.state_dict(),
                       root + model + 'discriminatorA_param.pkl')
            torch.save(D_B.state_dict(),
                       root + model + 'discriminatorB_param.pkl')
            with open(root + model + 'train_hist.pkl', 'wb') as f:
                pickle.dump(train_hist, f)

            util.show_train_hist(train_hist, save=True,
